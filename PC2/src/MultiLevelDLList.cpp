...
template<class T>
MultiLevelDLList<T>::~MultiLevelDLList() {
    for (auto &p : children) delete p.second;
}

template<class T>
void MultiLevelDLList<T>::incrementUp(int k) {
    auto* p = this;
    while (p) {
        p->m += k;
        p = p->parent;
...
template<class T>
void MultiLevelDLList<T>::decrementUp(int k) {
    auto* p = this;
    while (p) {
        p->m -= k;
        p = p->parent;
...
template<class T>
void MultiLevelDLList<T>::add(int i, const T& x) {
    assert(0 <= i && i <= mMax);
    DLList<T>::add(i, x);
    incrementUp(1);
}
...
template<class T>
void MultiLevelDLList<T>::add(const T& x) {
    DLList<T>::add(this->n, x);
    incrementUp(1);
}

template<class T>
MultiLevelDLList<T>* MultiLevelDLList<T>::addChild(int i) {
    Node* u = this->getNode(i);

    if (getChild(u)) return nullptr;
...
template<class T>
bool MultiLevelDLList<T>::hasChild(int i) {
    return getChild(this->getNode(i)) != nullptr;
}

template<class T>
void MultiLevelDLList<T>::removeNode(Node* w) {
    int removed = 1;
...
template<class T>
T MultiLevelDLList<T>::remove(int i) {
    assert(0 <= i && i < this->n);
...
template<class T>
int MultiLevelDLList<T>::size() const {
    return m;
}

template<class T>
bool MultiLevelDLList<T>::checkSize() const {
    int total = 0;
...
template<class T>
MultiLevelDLList<T>& MultiLevelDLList<T>::buildFromString(const std::string& input) {
    auto input_parsed = parse(input);
    this->clear();

    if (input_parsed.empty()) return *this;
    ...
    while (!stack.empty() && i < input_parsed.size()) {
        ...
        for (int v : values) child->add(v);

        for (int j = child->n - 1; j >= 0; --j) {
            stack.push_back({child, j});
        }

        if (i < input_parsed.size()) i++;
    }

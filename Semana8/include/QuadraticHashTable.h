...
class QuadraticHashTable {
    ...
    std::size_t index(const T& x, std::size_t i) const {
        return (hashCode(x) + i * i) % table_.size();
    }
...
public:
    explicit QuadraticHashTable(std::size_t c = 11, RehashPolicy policy = {})
        : table_(nextPrime(std::max<std::size_t>(policy.minCapacity, c))), policy_(policy) {
        policy_.maxLoad = std::min(policy_.maxLoad, 0.50);  // garantiza buen comportamiento del sondeo cuadratico
        policy_.maxOccupiedLoad = std::min(policy_.maxOccupiedLoad, 0.60);
        policy_.validate();
    }

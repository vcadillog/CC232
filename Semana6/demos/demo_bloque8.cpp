#include <ios>
#include <iostream>
#include <vector>

#include "Capitulo6.h"

namespace {

template <typename T>
void printVector(const std::vector<T> &xs, const char *label) {
  std::cout << label << ": [";
  for (std::size_t i = 0; i < xs.size(); ++i) {
    if (i != 0)
      std::cout << ", ";
    std::cout << xs[i];
  }
  std::cout << "]\n";
}

} // namespace

int main() {
  ods::PQ_LeftHeap<int> a{7, 2, 9};
  ods::PQ_LeftHeap<int> b{1, 8, 3, 11};

  printVector(a.levelOrder(), "heap A antes del merge");
  printVector(b.levelOrder(), "heap B antes del merge");

  std::cout << "Heap A es izquierdista: " << std::boolalpha
            << a.isValidLeftHeap() << "\n";
  std::cout << "Heap B es izquierdista: " << std::boolalpha
            << b.isValidLeftHeap() << "\n";

  a.merge(b);
  printVector(a.levelOrder(), "heap A despues del merge");
  std::cout << "Heap A es izquierdista: " << std::boolalpha
            << a.isValidLeftHeap() << "\n";

  std::cout << "B queda vacio: " << std::boolalpha << b.empty() << "\n";

  ods::PQ_LeftHeap<int> copia{11, 9, 8, 1, 3, 7, 2};
  printVector(copia.levelOrder(), "Copia de A despues del merge");

  a.insertMerge(10);
  copia.insertMerge(10);
  printVector(a.levelOrder(), "A despues de insert(10)");
  printVector(copia.levelOrder(), "A despues de insertMerge(10)");

  std::cout << "Heap A es izquierdista: " << std::boolalpha
            << a.isValidLeftHeap() << "\n";

  while (!a.empty()) {
    int y = a.delMaxLeftist();
    std::cout << "delMax() -> " << y << "\n";
    printVector(a.levelOrder(), "heap después de delMax()");
  }
  std::cout << '\n';
}

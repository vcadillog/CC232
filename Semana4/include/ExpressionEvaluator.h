...
inline bool isUnaryMinus(const std::string &expr, std::size_t i) {
  ...
  if (!(std::isdigit(static_cast<unsigned char>(expr[i + 1])) ||
        expr[i + 1] == '.')) {
    return false;
  }
  if (i == 0) {
...
inline long long factorialInt(int n) {
  if (n < 0) {
    throw std::runtime_error(
        "el factorial no se define para enteros negativos");
  }
  long long ans = 1;
  for (int i = 2; i <= n; ++i) {
    ans *= i;
...
inline double calcu(double a, char op, double b) {
  ...
  case '%':
    if (b == 0) {
      throw std::runtime_error("Módulo cero");
    }
    return int(a) % int(b);
  default:
...
inline void readNumber(const std::string &expr, std::size_t &i,
